...
class NewPostModal extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.img) {
      ...
      reader.onload = () => this.setState({ img: reader.result });
  ...
  displayNextStep = () => {
    if (this.state.text.length > 0) {
      this.setState(prevState => ({ secondStage: !prevState.secondStage }));
    }
  };
  ...
  publishPost = async () => {
    this.setState({ showLoader: true, secondStage: true });
    let newPost = this.state;
    delete newPost.showAdvanced;
    delete newPost.secondStage;
    delete newPost.showLoader;
    delete newPost.img;

    newPost.userID = this.props.user._id;
    const postJson = JSON.stringify({ ...newPost });

    let data = new FormData();
    data.append("img", this.imgData);
    data.append("text", postJson);
        ...
        <div>
          ...
          <PostBox ref={PostModal => (this.PostModal = PostModal)}>
            <Header>
              ...
              <CloseWindow onClick={() => this.props.showModal()}>
                ╳
              </CloseWindow>
            </Header>
            ...
            <HashtagDiv>
              <HashtagButton onClick={this.addHash}>
                + Add hashtag
              </HashtagButton>
        ...
        <div>
          ...
          <PostBox ref={PostModal => (this.PostModal = PostModal)}>
            ...
            <AllowComment show={this.state.showAdvanced}>
              <AllowText show={this.state.showAdvanced}>
              ...
              <AllowButtonDiv show={this.state.showAdvanced}>
                ...
                <AllowButton
                  switch={this.state.allowComments}
                  onClick={this.allowCommentsOnPage}
                >
                  <AllowButtonSpan />
                </AllowButton>
              </AllowButtonDiv>
            ...
            <ButtonsDiv>
              ...
              <PostButton onClick={this.publishPost}>
                {this.state.showLoader && <Loader />}Post
              </PostButton>
            </ButtonsDiv>
          </PostBox>
        </div>
...
const OptionDiv = styled.div`
  color: ${props =>
    props.selected === props.name ? "#0084bf" : "rgba(0,0,0, .6)"};
  width: 100%;
  height: 5rem;
  background-color: white;
  display: flex;
...
const BackButton = styled(PostButton)`
  background-color: white;
  color: #0084bf;
  box-shadow: inset 0 0 0 0.1rem #0084bf;
  margin-right: 1rem;

  &:hover {
    background-color: rgba(152, 216, 244, 0.25);
    color: #006097;
    box-shadow: inset 0 0 0 0.2rem #006097;
  }
`;
