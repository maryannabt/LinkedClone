...
class Social extends Component {
  ...
  changeDisplaySort(sortValue) {
    this.setState({ sortBy: sortValue, display: false });
  }
  ...
  uploadLike = () => {
    ...
    this.props.updateLike(newLikeJson);
    ...
    return (
      <Main>
        ...
        <SortComments show={this.state.display}>
          <Top
            onClick={() => this.changeDisplaySort("Top")}
            selected={this.state.sortBy}
          >
            Top Comments
          </Top>
          <Recent
            onClick={() => this.changeDisplaySort("Recent")}
            selected={this.state.sortBy}
          >
            Recent Comments
          </Recent>
...
const Recent = styled(Top)`
  border: none;
  color: ${props =>
    props.selected === "Recent" ? "#0073b1" : "rgba(0,0,0,.9)"};
`;
