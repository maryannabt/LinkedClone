/* Rendered in Links component and togggled by a click.
   Props: user - currently logged in user. Value derived from the redux store state as a result of loginData reducer operation.
          hideMenu - function defined in Links component, that changes the state in Links component and allows to hide the Menu.
          ClickMeRef - a reference to DOM element LinkMe (div) in the Links component, so the Menu component knows when it is clicked.
...
class Menu extends Component {
  ...
  removeMenu = e => {
    if (
      !this.LinkMenu.contains(e.target) &&
      !this.props.ClickMeRef.contains(e.target)
    ) {
      this.props.hideMenu();
    }
  };
  ...
  render() {
    ...
    return (
      <Main ref={LinkMenu => (this.LinkMenu = LinkMenu)}>
        <Link
          to={`/user/${_id}`}
        ...
        <VBox>
          ...
          <Signout onClick={() => this.props.logUserOut()}>
            <span>Sign out</span>
          </Signout>
        </VBox>
      </Main>
