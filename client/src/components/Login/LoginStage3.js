/* Rendered in App component with path "/start/photo". Connected to the redux store.
   Dispatches an action finishTutorial (or finishTutorialNoAvatar) that takes in an image file submitted by the user through the form. */

import React, { Component } from "react";
import styled from "styled-components";
import CameraBG from "../../img/login_stage3_camera_icon.png";
import { connect } from "react-redux";
import {
  finishTutorial,
  finishTutorialNoAvatar
...
class LoginStage3 extends Component {
  ...
  uploadAvatar = event => {
    ...
    // Event handler executed when content read with readAsDataURL is available.
    // reader.result is base64 encoded string that we can display as image.
    reader.onload = e => this.setState({ imgBG: reader.result });
...
const Button = styled.button`
  ...
  background-color: ${props =>
    props.finish === null ? "rgb(0, 115, 177)" : "#0073b1"};
